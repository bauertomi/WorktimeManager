...
public class NetworkDiscover {
	...
	public interface NetworkDiscoverListener {
		 ...
		 */
		public void onDiscover(InetAddress address,
	...
	public void startListening() {
		...
		if (mDiscoverWatchdogThread == null
				|| !mDiscoverWatchdogThread.isAlive()) {
	...
	private void addInetAddress(InetAddress address) {
		...
		synchronized (mAddresses) {
			...
			if (!mAddresses.containsKey(address)
					|| mAddresses.get(address) < currentTime - TIMEOUT) {
				newAddress = true;
			}
			...
			if (mSocket != null) {
				while (true) {
					try {
						...
						String str = new String(mBuffer, mPacket.getOffset(),
								mPacket.getLength());
